...
if query_string := st.chat_input("请输入具体问题："):
    ...
    if current_session["title"] == "🆕 新建会话":
        ...
        new_title = query_string[:10] + "..." if len(query_string) > 5 else query_string 
    ...
    with st.chat_message("assistant"):
        ...
        if financial_agent is None:
            ...
            try:
                ...
                with st.status("Agent 正在思考中...", expanded=True) as status:
                    ...
                    if steps:
                        for action, observation in response["intermediate_steps"]:
                            st.write(f"🛠️ 尝试调用工具: `{action.tool}`")
                            st.caption(f"工具返回内容摘要: {str(observation)[:100]}...")
                ...
                if extracted_answer:
                    ...
                    # 使用 session_manager 将对话存入 chat_history
                    sm.add_message_to_current("assistant", extracted_answer)

            except Exception as api_exception:
                st.error(f"运行出错: {str(api_exception)}")
                traceback.print_exc()