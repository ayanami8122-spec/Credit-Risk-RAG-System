...
def get_rag_chain():
    ...
    system_prompt =(
        ...
        "3. 如果知识库已经提供相关内容，不要依赖自身知识进行推测或补充。\n"
        "4. 如果知识库没有相关内容，明确说明知识库未包含该信息。\n\n"
        "【当前知识库】:\n"
        "{context}"
    )
...
def warmup_rag():
    ...
    if _GLOBAL_RAG_CHAIN is None:
        ...
        _GLOBAL_RAG_CHAIN = get_rag_chain()
...
def get_financial_knowledge(query: str) -> str:
    ...
    """
    该工具专门用于查询金融风险控制、信用违约概率（PD）、违约损失率（LGD）、
    以及金融行业监管合规相关的专业知识。
    当用户的问题设计专业金融概念时，请务必调用此工具。
    """
    ...
    # PS: Agent 调用工具一般是同步的，所以我们在此获取完整回答
    # 流式在 AgentExecutor 层处理
    response = _GLOBAL_RAG_CHAIN.invoke({"input": query})
    return response["answer"]

# 导出工具给 Agent 使用
financial_tool = Tool(

    name = "Financial_Risk_Knowledge_Base",
    func = get_financial_knowledge,
    description = """
    用于查询金融风险管理专业知识。
